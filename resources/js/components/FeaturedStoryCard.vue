<template>
    <router-link
        class="bg-current px-5 pb-5 relative shrink-when-active mb-8"
        :to="`/stories/${story.id}`"
        :style="{
    ...
    >
        <div class="text-black h-full overflow-hidden">
            <div class="flex font-display font-bold text-base uppercase my-4">
                {{ truncate(place.name, { length: 35 }) }}
            </div>
            <quote-icon class="text-black w-5 h-5" />

            <p
                class="leading-loose mt-5 text-sm xl:text-base xl:leading-normal"
            >
                {{ story.content }}
            </p>
            ...
            >
                <h3 class="self-end text-xs xl:text-2xs font-bold">
                    {{ story.role }}
                </h3>
