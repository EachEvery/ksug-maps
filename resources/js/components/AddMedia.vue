<template>
  <div class="flex flex-col xl:flex-row font-mono bg-gray-100 py-8 px-6 items-start">
    <camera-icon class="w-10 h-6 mr-4 mb-5 opacity-75" />
    ...
    <div class="flex flex-col">
      ...
      <div class="h-24 flex flex-col items-stretch justify-center -mb-4">
        <div class="flex justify-between items-center" v-if="file">
          <span class="bg-white p-2 text-2xs shadow-inner border border-gray-600">{{ fileName }}</span>
        ...
        <span
          class="italic text-2xs mt-2"
          style="max-width: 15rem"
        >Due to browser restrictions, we only accept MP4 video files.</span>
      </div>
    </div>
  </div>
</template>
